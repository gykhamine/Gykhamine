...
class RuntimeManager:
    ...
    def auto_discover_actions(self, app):
        """Scanne le code de l'application (app.py, control_panel, file_panel,
        editor_view, terminal_panel) pour enregistrer automatiquement toutes les
        méthodes-actions déjà existantes (boutons) comme actions runtime
        disponibles, sans dupliquer celles déjà enregistrées manuellement via
        register_action(). Ne réécrit rien : ne fait qu'exposer ce qui existe
        déjà dans le code au système de runtime."""
        import inspect

        # Widgets/objets de l'app à inspecter, avec un préfixe lisible pour
        # distinguer l'origine de chaque action découverte.
        targets = [("App", app)]
        for attr_name, prefix in [
            ("control_panel", "Contrôles"),
            ("file_panel", "Explorateur"),
            ("editor_view", "Éditeur"),
            ("terminal_panel", "Terminal"),
        ]:
            obj = getattr(app, attr_name, None)
            if obj is not None:
                targets.append((prefix, obj))

        # Méthodes exclues car destructives, bloquantes, ou nécessitant des
        # arguments/contexte qu'une automatisation runtime ne peut pas fournir.
        excluded_keywords = (
            "delete", "supprim", "rename", "renam", "destroy", "quit",
            "close", "cancel", "popup", "popdown", "dialog", "picker",
            "context_menu", "on_row", "on_right_click", "key_pressed",
            "cell_data", "drag", "drop", "get_", "_load", "_save",
            "_build", "_init", "_setup", "_on_search", "_apply_search",
        )
        ...
        for prefix, obj in targets:
            for name, member in inspect.getmembers(obj, predicate=inspect.ismethod):
                ...
                try:
                    ...
                    continue
                # N'accepte que les méthodes appelables sans argument requis
                # (hors *args/**kwargs), pour rester compatible avec run()
                # qui les invoque via GLib.idle_add(cb) sans paramètre.
                params = [p for p in sig.parameters.values()
                          if p.kind not in (p.VAR_POSITIONAL, p.VAR_KEYWORD)]
                if any(p.default is p.empty for p in params):
                    continue

                action_id = f"auto::{prefix}::{name}"
                if action_id in self._actions:
                    continue
                label = f"{prefix} — {name.lstrip('_').replace('_', ' ')}"
                self.register_action(action_id, label, member)
                discovered += 1

        if discovered:
            global_log(f"⚡ Runtime: {discovered} action(s) découverte(s) automatiquement depuis le code de l'app.")
        return discovered
