...
import re, requests, threading, subprocess
...
class BlockAIEngine:
    ...
    def _build_prompt(self, block_type, current_code, user_intent, context_deps="", mode="modify", custom_role=None):
        ...
        role = custom_role if custom_role else prompts.BLOCK_TYPE_ROLES.get(block_type, prompts.BLOCK_TYPE_ROLES["other"])
        format_instruction = prompts.FORMAT_INSTRUCTIONS.get(mode, prompts.FORMAT_INSTRUCTIONS["default"])

        prompt = prompts.PROMPT_TEMPLATE.format(
            role=role,
            context_deps=context_deps if context_deps else "Aucune dépendance externe majeure.",
    ...
    def process_modification(self, block_type, current_code, user_intent, context_deps="", mode="modify", custom_role=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # log_analysis et business_process : texte libre conservé intégralement, sans filtre JSON
                if mode in ["log_analysis", "business_process"]:
                    return raw_content.strip()
                return self._clean_code_output(raw_content, block_type if mode != "terminal_gen" else "shell")
            else:
...
class AIModificationDialog(Gtk.Dialog):
    def __init__(self, parent, block, ai_engine, on_confirm_cb, project_root=None):
        ...
        apply_dark_source_scheme(buf_orig)
        ...
        apply_dark_source_scheme(buf_new)
    ...
    def _thread_generate(self, btype, intent):
        ...
        if cached:
            ...
            self._pending_cache_intent = None  # déjà en cache, rien à valider
            GLib.idle_add(lambda: self.ai_engine.log("📂 Modification récupérée depuis la DB (Cache Code)"))
        else:
            # 2. Appel IA — pas de sauvegarde automatique : attend la validation (bouton ✅ Conserver)
            result = self.ai_engine.process_modification(btype, self.block['code'], intent, self.context_deps, mode="contextual_modify")
            self._pending_cache_intent = (intent, btype) if result else None
    ...
    def _on_accept(self, *_):
        if self.modified_code:
            # Sauvegarde en cache uniquement à la validation, pour éviter de conserver
            # une information générée par l'IA mais non vérifiée par l'utilisateur.
            pending = getattr(self, '_pending_cache_intent', None)
            if pending:
                intent, btype = pending
                save_block_to_cache(intent, self.modified_code, block_type=btype)
                self.ai_engine.log("💾 Modification validée et sauvegardée (Cache Code).")
                self._pending_cache_intent = None
...
class LogAnalyzerDialog(Gtk.Dialog):
    def __init__(self, parent, ai_engine, log_callback):
        ...
        apply_dark_source_scheme(self.txt_logs.get_buffer())
        ...
        apply_dark_source_scheme(self.txt_result.get_buffer())
    ...
    def _on_analyze(self, *_):
        ...
        def _thread():
            ...
            if result:
                GLib.idle_add(lambda: self.txt_result.get_buffer().set_text(result))
...
class AICmdGeneratorDialog(Gtk.Dialog):
    def __init__(self, parent, terminal_panel):
        ...
        self.btn_validate = Gtk.Button(label="✅ Valider")
        self.btn_validate.set_tooltip_text("Confirmer que la commande est correcte et la garder en cache")
        self.btn_validate.set_sensitive(False)
        self.btn_validate.connect("clicked", self._on_validate_cache)
        ...
        action_box.append(self.btn_validate)
    ...
    def _on_translate(self, *_):
        ...
        def _thread():
            ...
            if cached:
                ...
                if cmd:
                    # Pas de sauvegarde automatique : attend la validation de l'utilisateur
                    self._pending_cache = (intent, self.is_process_mode)
                    GLib.idle_add(lambda: self.terminal_panel._log("🤖 Commande générée — cliquez sur ✅ Valider pour la garder en cache."))
                else:
            ...
            GLib.idle_add(lambda: (
                ...
                self.btn_validate.set_sensitive(not cached),
    ...
    def _on_validate_cache(self, *_):
        """Sauvegarde la commande générée en cache seulement après validation explicite,
        pour éviter de conserver de fausses informations non vérifiées."""
        if hasattr(self, '_pending_cache') and self._pending_cache and self.generated_cmd:
            intent, is_process = self._pending_cache
            save_command_to_cache(intent, self.generated_cmd, is_process)
            self.terminal_panel._log("💾 Commande validée et sauvegardée en cache.")
            self.btn_validate.set_sensitive(False)
...
class GitManagerDialog(Gtk.Dialog):
    def __init__(self, parent, project_root, log_callback):
        ...
        apply_dark_source_scheme(self.txt_log.get_buffer())
...
class BusinessProcessDialog(Gtk.Dialog):
    def __init__(self, parent, ai_engine, log_callback, config_getter=None):
        ...
        # Rôles par défaut — réponse en texte libre, aucun format imposé
        # (définis dans prompts.py pour faciliter leur maintenance indépendamment du code)
        self.default_roles = dict(prompts.BUSINESS_PROCESS_DEFAULT_ROLES)
        ...
        apply_dark_source_scheme(self.txt_problem.get_buffer())
        ...
        self.btn_validate = Gtk.Button(label="✅ Valider")
        self.btn_validate.set_tooltip_text("Confirmer la réponse et la garder en cache")
        self.btn_validate.set_sensitive(False)
        self.btn_validate.connect("clicked", self._on_validate_cache)
    ...
    def _on_generate(self, *_):
        ...
        selected_role_prompt = self.default_roles.get(active_text) or self.custom_roles.get(active_text, prompts.FALLBACK_ROLE_PROMPT)
        
        self.txt_result.get_buffer().set_text("Recherche dans le cache...")
        
        def _thread():
            # 1. Vérifier le cache des processus
            cached = get_cached_process(problem)
            if cached:
                result = cached["json_content"]
                self._pending_cache = None  # déjà en cache, rien à valider
                GLib.idle_add(lambda: self.log_callback("📂 Réponse récupérée depuis la DB (Cache Processus)"))
            else:
                ...
                try:
                    ...
                    # Pas de sauvegarde automatique : attend la validation de l'utilisateur
                    self._pending_cache = (problem, result, active_text)
                    GLib.idle_add(lambda: self.log_callback("🤖 Réponse générée — cliquez sur ✅ Valider pour la garder en cache."))
            ...
            # Texte intégral conservé, aucun filtre JSON
            GLib.idle_add(lambda: self.txt_result.get_buffer().set_text(result))
            GLib.idle_add(lambda: self.log_callback(f"📊 RÉPONSE GÉNÉRÉE (Rôle: {active_text})."))
            GLib.idle_add(lambda: self.btn_validate.set_sensitive(bool(self._pending_cache)))
    ...
    def _on_validate_cache(self, *_):
        """Sauvegarde la réponse générée en cache seulement après validation explicite,
        pour éviter de conserver de fausses informations non vérifiées."""
        if getattr(self, '_pending_cache', None):
            problem, result, role = self._pending_cache
            save_process_to_cache(problem, result, role_type=role)
            self.log_callback("💾 Réponse validée et sauvegardée en cache.")
            self.btn_validate.set_sensitive(False)
            self._pending_cache = None
