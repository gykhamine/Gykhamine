...
class BlockEditorView(Gtk.Box):
    def __init__(self, toast_cb, run_file_cb, get_config_cb=None, ai_engine=None, status_update_cb=None, refresh_explorer_cb=None, get_project_root_cb=None):
        ...
        self.refresh_explorer_cb = refresh_explorer_cb
    ...
    def _build_toolbar(self):
        ...
        self.block_search_entry = Gtk.Entry()
        self.block_search_entry.set_placeholder_text("🔍 Rechercher un bloc (nom/code)…")
        self.block_search_entry.set_hexpand(True)
        self.block_search_entry.connect("changed", self._on_block_search_changed)
        bar.append(self.block_search_entry)
    ...
    def _open_c_compiler(self, *_):
        dialog = CCompilerDialog(
            self.get_root(), self._get_config_cb, self.toast_cb,
            get_project_root_cb=self.get_project_root_cb,
            get_c_blocks_cb=lambda: self.blocks,
        )
        dialog.present()

    def _open_c_compiler_for_block(self, block):
        default_name = re.sub(r"[^a-zA-Z0-9_\-]", "_", block.get("name", "") or "gy_c") or "gy_c"
        dialog = CCompilerDialog(
            self.get_root(), self._get_config_cb, self.toast_cb,
            get_project_root_cb=self.get_project_root_cb,
    ...
    def _on_block_search_changed(self, entry):
        """Filtre les cartes de blocs selon le nom ou le contenu du code, ouvre
        automatiquement tout bloc correspondant (nom et/ou code) pour que le texte
        trouvé soit visible en couleur directement, referme systématiquement tous
        les blocs non concernés, et surligne le mot recherché à l'intérieur du
        code (comme un Ctrl+F)."""
        query = entry.get_text().strip().lower()
        match_count = 0
        self._suppress_save_toast = True
        try:
            for card in self._cards:
                ...
                if not query:
                    ...
                    card.highlight_name_match(None)
                    if getattr(card, "_auto_expanded_by_search", False):
                        if card.expanded:
                            card._toggle_edit()
                        card._auto_expanded_by_search = False
                    continue
                name = block.get("name", "").lower()
                code = block.get("code", "").lower()
                match = query in name or query in code
                card.set_visible(match)
                if match:
                    match_count += 1
                    card.add_css_class("block-search-match")
                    card.highlight_name_match(query if query in name else None)
                    # Ouvrir le bloc EN PREMIER pour que le widget soit réalisé,
                    # puis appliquer le surlignage (highlight_search gère le timing).
                    if not card.expanded:
                        card._auto_expanded_by_search = True
                        card._toggle_edit()
                    if query in code:
                        card.highlight_search(query)
                    else:
                        card.clear_search_highlight()
                else:
                    card.remove_css_class("block-search-match")
                    card.clear_search_highlight()
                    card.highlight_name_match(None)
                    # Tout bloc non concerné est refermé, qu'il ait été ouvert par la
                    # recherche ou manuellement par l'utilisateur avant de chercher.
                    if card.expanded:
                        card._toggle_edit()
                    card._auto_expanded_by_search = False
        finally:
            self._suppress_save_toast = False
    ...
    def _add_block_dialog(self, *_):
        ...
        enable_window_controls(dialog, "Ajouter un bloc")
    ...
    def _render_blocks_recursive(self, blocks, container, level=0, parent_prefix=""):
        ...
        for index, block in enumerate(blocks):
            ...
            card = BlockCard(
                ...
                parent_window=self,
                on_compile_cb=self._open_c_compiler_for_block
            )
    ...
    def _on_block_save(self, block, new_code):
        ...
        self._push_state()
        self._modified = True
        if not getattr(self, "_suppress_save_toast", False):
    ...
    def _save_file(self, *_):
        ...
        try:
            ...
            if self.refresh_explorer_cb:
                self.refresh_explorer_cb()
