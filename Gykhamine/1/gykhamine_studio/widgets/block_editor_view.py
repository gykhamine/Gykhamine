...
class BlockEditorView(Gtk.Box):
    ...
    def _goto_search_match(self, index: int, query: str = None):
        ...
        if card is not None:
            # Résultat dans le fichier actif : replie tous les autres blocs puis
            # déplie et surligne uniquement celui qui contient le résultat.
            for c in self._cards:
                if c is not card:
                    c.collapse()
            card.reveal_search_match(query)
        elif path_str:
            # Résultat dans un autre onglet : bascule dessus, replie tous ses
            # blocs, puis déplie/surligne uniquement celui qui correspond.
