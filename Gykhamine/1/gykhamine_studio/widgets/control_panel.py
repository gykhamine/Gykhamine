...
from .db_manager_dialog import DbManagerDialog
...
class ControlPanel(Gtk.Box):
    def __init__(self, get_project_root, get_config, show_toast, terminal_panel, on_export_gyboard=None, on_open_gyboard=None, on_save_gyboard=None, on_recent_changed=None, on_open_in_editor=None):
        ...
        self.on_export_gyboard, self.on_open_gyboard, self.on_save_gyboard = on_export_gyboard, on_open_gyboard, on_save_gyboard
    ...
    def _build(self):
        ...
        commands = [("🆕 startproject", "startproject"), ("📐 makemigrations", "makemigrations"), ("⬆ migrate", "migrate"), ("👤 superuser", "createsuperuser"), ("📱 New App", "startapp"), ("🐚 shell", "shell"), ("🗄 dbshell", "dbshell"), ("📦 collectstatic", "collectstatic"), ("✅ check", "check"), ("📜 showmigrations", "showmigrations"), ("🧹 flush", "flush")]
        for idx, (label, cmd) in enumerate(commands):
            ...
            if cmd == "createsuperuser": 
                ...
                btn.connect("clicked", lambda *_: self._show_startproject_dialog())
            else: 
        ...
        lbl_db = Gtk.Label(label="⚙️ Configuration"); lbl_db.add_css_class("control-section-title"); lbl_db.set_xalign(0); self.append(lbl_db)

        btn_prompts = Gtk.Button(label="⚙️ Gestion des Configurations (Prompts IA)")
        btn_prompts.add_css_class("ctrl-btn"); btn_prompts.set_hexpand(True)
        btn_prompts.set_tooltip_text("Gérer les prompts/configurations IA stockés dans la base système (gy_studio.db) — pas la base du projet Django")
        btn_prompts.connect("clicked", self._open_prompt_manager)
        self.append(btn_prompts)

        btn_cache = Gtk.Button(label="🧠 Gestion du Cache IA")
        btn_cache.add_css_class("ctrl-btn"); btn_cache.set_hexpand(True)
        btn_cache.set_tooltip_text("Consulter/vider le cache des commandes, blocs de code et processus générés par l'IA")
        btn_cache.connect("clicked", self._open_cache_manager)
        self.append(btn_cache)

        btn_db_manager = Gtk.Button(label="🗄 Gestionnaire DB (Projets récents, Mémoire, Config, Logs)")
        btn_db_manager.add_css_class("ctrl-btn"); btn_db_manager.set_hexpand(True)
        btn_db_manager.set_tooltip_text("Consulter/nettoyer la base système (gy_studio.db) : projets récents, historique mémoire, clés de config et fichier de log")
        btn_db_manager.connect("clicked", self._open_db_manager)
        self.append(btn_db_manager)

        lbl_db_django = Gtk.Label(label="🗄 Base de données (Projet Django)"); lbl_db_django.add_css_class("control-section-title"); lbl_db_django.set_xalign(0); lbl_db_django.set_margin_top(8); self.append(lbl_db_django)
        ...
        lbl_gyboard = Gtk.Label(label="📦 Distribution (.gyboard)"); lbl_gyboard.add_css_class("control-section-title"); lbl_gyboard.set_xalign(0); lbl_gyboard.set_margin_top(8); self.append(lbl_gyboard)
        lbl_gyboard_hint = Gtk.Label(label="Un seul fichier livrable (comme un .pbix) : code + base SQLite, au lieu d'un dossier entier."); lbl_gyboard_hint.add_css_class("dim-label"); lbl_gyboard_hint.set_xalign(0); lbl_gyboard_hint.set_wrap(True); self.append(lbl_gyboard_hint)
        btn_export_gyboard = Gtk.Button(label="📦 Exporter en .gyboard")
        btn_export_gyboard.add_css_class("ctrl-btn"); btn_export_gyboard.set_hexpand(True)
        btn_export_gyboard.set_tooltip_text("Empaqueter le projet ouvert en un seul fichier .gyboard")
        if self.on_export_gyboard: btn_export_gyboard.connect("clicked", self.on_export_gyboard)
        self.append(btn_export_gyboard)
        btn_open_gyboard = Gtk.Button(label="📂 Ouvrir un .gyboard")
        btn_open_gyboard.add_css_class("ctrl-btn"); btn_open_gyboard.set_hexpand(True)
        btn_open_gyboard.set_tooltip_text("Décompresser un .gyboard en mémoire et l'exécuter/éditer réellement")
        if self.on_open_gyboard: btn_open_gyboard.connect("clicked", self.on_open_gyboard)
        self.append(btn_open_gyboard)
        btn_save_gyboard = Gtk.Button(label="💾 Sauvegarder vers .gyboard")
        btn_save_gyboard.add_css_class("ctrl-btn"); btn_save_gyboard.set_hexpand(True)
        btn_save_gyboard.set_tooltip_text("Recompresser le dossier de travail (code + données modifiés) dans le .gyboard d'origine")
        if self.on_save_gyboard: btn_save_gyboard.connect("clicked", self.on_save_gyboard)
        self.append(btn_save_gyboard)
        ...
        pg_backup_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        btn_pg_backup = Gtk.Button(label="💾 Sauvegarder (zip)")
        btn_pg_backup.add_css_class("ctrl-btn"); btn_pg_backup.set_hexpand(True)
        btn_pg_backup.set_tooltip_text("Sauvegarde complète de la base PostgreSQL (pg_dump) compressée en .zip")
        btn_pg_backup.connect("clicked", self._run_pg_backup)
        pg_backup_box.append(btn_pg_backup)

        btn_pg_restore = Gtk.Button(label="♻️ Restaurer")
        btn_pg_restore.add_css_class("ctrl-btn"); btn_pg_restore.set_hexpand(True)
        btn_pg_restore.set_tooltip_text("Restaure la base depuis une archive .zip créée par « Sauvegarder »")
        btn_pg_restore.connect("clicked", self._run_pg_restore)
        pg_backup_box.append(btn_pg_restore)

        btn_pg_reset = Gtk.Button(label="🗑 Réinitialiser")
        btn_pg_reset.add_css_class("ctrl-btn-warn"); btn_pg_reset.set_hexpand(True)
        btn_pg_reset.set_tooltip_text("Supprime puis recrée la base VIDE (toutes les données seront perdues)")
        btn_pg_reset.connect("clicked", self._run_pg_reset)
        pg_backup_box.append(btn_pg_reset)
        self.append(pg_backup_box)
        ...
        sep_ext = Gtk.Separator(); sep_ext.set_margin_top(8); sep_ext.set_margin_bottom(4); self.append(sep_ext)
        lbl_ext = Gtk.Label(label="🎮 Outils Externes"); lbl_ext.add_css_class("control-section-title"); lbl_ext.set_xalign(0); self.append(lbl_ext)

        btn_antimicrox = Gtk.Button(label="🎮 Lancer AntiMicroX (hidden)")
        btn_antimicrox.add_css_class("ctrl-btn"); btn_antimicrox.set_hexpand(True)
        btn_antimicrox.set_tooltip_text("Démarrer AntiMicroX en arrière-plan (--hidden) avec un profil de configuration")
        btn_antimicrox.connect("clicked", self._show_antimicrox_dialog)
        self.append(btn_antimicrox)

        btn_pip_folder = Gtk.Button(label="📦 Installer des paquets depuis un dossier")
        btn_pip_folder.add_css_class("ctrl-btn"); btn_pip_folder.set_hexpand(True)
        btn_pip_folder.set_tooltip_text("pip install de tous les .whl/.tar.gz/.zip trouvés dans un dossier (utile en offline)")
        btn_pip_folder.connect("clicked", self._show_pip_install_folder_dialog)
        self.append(btn_pip_folder)
        ...
        cfg_paths = self.get_config()
        self.gy_paths = {
            "sudo": cfg_paths.get("gy_capsule_path_1", "Gykhamine/1/gy.py"),
            "user": cfg_paths.get("gy_capsule_path_2", "Gykhamine/2/gy.py"),
        }
        for key, label, sudo in [("sudo", "🔑 /1/gy.py", True), ("user", "👤 /2/gy.py", False)]:
            row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            entry = Gtk.Entry(); entry.set_text(self.gy_paths[key]); entry.set_hexpand(True)
            entry.set_tooltip_text("Chemin (relatif à la racine du projet) vers le script à exécuter")
            entry.connect("changed", lambda e, k=key: self._on_gy_path_changed(k, e))
            btn = Gtk.Button(label=f"Run {label}"); btn.add_css_class("ctrl-btn-warn" if sudo else "ctrl-btn")
            btn.connect("clicked", lambda *_, k=key, s=sudo: self._run_gy(self.gy_paths[k], s))
            btn_edit = Gtk.Button(icon_name="document-edit-symbolic")
            btn_edit.set_tooltip_text("Open in Block Editor")
            btn_edit.add_css_class("ctrl-btn-small")
            btn_edit.connect("clicked", lambda *_, k=key: self._open_gy_in_editor(k))
            row.append(entry); row.append(btn); row.append(btn_edit)
            self.append(row)

        autorun_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        autorun_row.set_margin_top(4)
        self.switch_capsule_autorun = Gtk.Switch()
        self.switch_capsule_autorun.set_active(bool(cfg_paths.get("gy_capsule_autorun", False)))
        self.switch_capsule_autorun.set_valign(Gtk.Align.CENTER)
        self.switch_capsule_autorun.connect("state-set", self._on_capsule_autorun_toggled)
        autorun_row.append(self.switch_capsule_autorun)
        autorun_lbl = Gtk.Label(label="Auto-run /2/gy.py on app startup"); autorun_lbl.set_xalign(0)
        autorun_row.append(autorun_lbl)
        self.append(autorun_row)
    ...
    def _show_createsuperuser_dialog(self, *_):
        ...
        enable_window_controls(dialog, "Créer un Superutilisateur Django")
    ...
    def _show_startapp_dialog(self, *_):
        ...
        enable_window_controls(dialog, "Créer une nouvelle App Django")
    ...
    def _show_startproject_dialog(self, *_):
        """Ouvre un dialogue pour démarrer un nouveau projet Django (django-admin startproject)."""
        dialog = Gtk.Dialog(title="Créer un nouveau Projet Django (startproject)", transient_for=self.get_root())
        enable_window_controls(dialog, "Créer un nouveau Projet Django (startproject)")
        dialog.add_css_class("rounded-dialog")
        ...
        info_lbl = Gtk.Label(label="Entrez le nom du projet (ex: monerp). Il sera créé directement dans le dossier actuellement ouvert.", xalign=0, margin_bottom=8)
        info_lbl.add_css_class("dim-label")
        info_lbl.set_wrap(True)
        content.append(info_lbl)

        entry_name = Gtk.Entry()
        entry_name.set_placeholder_text("nom_du_projet")
        entry_name.set_activates_default(True)
        content.append(entry_name)
        ...
        def on_create(*_):
            project_name = entry_name.get_text().strip()
            if not project_name:
                self.show_toast("❌ Le nom du projet est requis")
                return
            if not re.match(r'^[a-zA-Z_]\w*$', project_name):
                self.show_toast("❌ Nom invalide (lettres, chiffres, underscores uniquement)")
                return
            self._run_startproject(project_name)
            dialog.destroy()
    ...
    def _run_startproject(self, project_name):
        """Exécute django-admin startproject dans le dossier actuellement ouvert."""
        root = self.get_project_root()
        if not root:
            self.show_toast("❌ Ouvrez d'abord un dossier de destination")
            return

        self.terminal._log(f"▶ Création du projet Django : {project_name}")
        self._run_cmd([sys.executable, "-m", "django", "startproject", project_name, "."], cwd=str(root))

        def refresh_explorer():
            if self.get_project_root():
                self.file_panel.load_project(self.get_project_root(), self.get_config())
                self.show_toast(f"✅ Projet '{project_name}' créé")
    ...
    def _kill_port_dialog(self, *_):
        ...
        enable_window_controls(dialog, "Kill a process")
        ...
        def on_kill(*_):
            try:
                ...
                if kill_process_on_port(port_num):
                    self.terminal._log("🔫 Process killed")
                    self.show_toast("Port freed")
                    # Mettre à jour les dots si le process tué correspond à un service connu
                    for name, proc in list(self.processes.items()):
                        if proc and proc.poll() is not None:
                            self.processes.pop(name, None)
                            self._set_dot(name, False)
                else:
                    self.terminal._log("⚠ No process found")
    ...
    def _ufw_allow_dialog(self, *_):
        ...
        enable_window_controls(dialog, "Ouvrir un port (UFW)")
    ...
    def _show_open_browser_dialog(self, *_):
        ...
        enable_window_controls(dialog, "🌐 URLs Accessibles")
        dialog.add_css_class("rounded-dialog")
...
# Dans la classe ControlPanel, remplacez la méthode _open_business_process par :
    ...
    def _show_system_manager_dialog(self, *_):
        ...
        enable_window_controls(dialog, "🛡️ Gestion Système & Réseau Avancée")
        dialog.add_css_class("rounded-dialog")
    ...
    def _kill_process(self, pid, name, popover):
        ...
        dlg = Adw.MessageDialog(
            ...
            heading=f"Tuer {name} ?",
            body=f"Le processus PID {pid} sera arrêté immédiatement."
        )

        dlg.add_response("cancel", "Annuler")
        dlg.add_response("kill", "Tuer")
...
"""
    ...
    def _run_pg_rundb(self, *_):
        ...
        def _thread():
            try:
                ...
                # 2. Montage — uniquement si un périphérique dédié est explicitement
                # renseigné (système persistant : /var est déjà un dossier local, pas
                # besoin de montage par défaut).
                ...
                if mount_check.returncode != 0:
                    if device:
                        ...
                        GLib.idle_add(self.terminal._log, "ℹ️ Aucun périphérique dédié : utilisation du dossier local.")
                else:
                    GLib.idle_add(self.terminal._log, "✅ Déjà monté")
    ...
    def _run_pg_backup(self, *_):
        cfg = self.get_config()
        db_name = cfg.get("pg_db_name", "ma_base")
        root = self.get_project_root()
        backups_dir = (root / "backups_db") if root else Path.home() / "backups_db"
        self.terminal._log(f"💾 === Sauvegarde de la base « {db_name} » ===")

        def _thread():
            try:
                backups_dir.mkdir(parents=True, exist_ok=True)
                stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                sql_path = backups_dir / f"{db_name}_{stamp}.sql"
                zip_path = backups_dir / f"{db_name}_{stamp}.zip"

                GLib.idle_add(self.terminal._log, f"▶ pg_dump de « {db_name} »...")
                # sudo -u postgres : même utilisateur système que les autres
                # commandes PostgreSQL de ce panneau (Init, Créer DB, ...).
                result = subprocess.run(
                    ["sudo", "-u", "postgres", "pg_dump", db_name],
                    capture_output=True, text=True,
                )
                if result.returncode != 0:
                    raise RuntimeError(result.stderr.strip() or "pg_dump a échoué")
                sql_path.write_text(result.stdout, encoding="utf-8")

                GLib.idle_add(self.terminal._log, "▶ Compression en .zip...")
                with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
                    zf.write(sql_path, arcname=sql_path.name)
                sql_path.unlink()  # on ne garde que le .zip, pas le .sql brut à côté

                GLib.idle_add(self.terminal._log, f"✅ Sauvegarde créée : {zip_path}")
                GLib.idle_add(self.show_toast, f"💾 Sauvegarde enregistrée : {zip_path.name}")
            except Exception as e:
                GLib.idle_add(self.terminal._log, f"❌ Erreur sauvegarde: {e}")
                GLib.idle_add(self.show_toast, "❌ Échec de la sauvegarde")

        threading.Thread(target=_thread, daemon=True).start()

    def _run_pg_restore(self, *_):
        Gtk.FileDialog(title="Choisir une archive .zip de sauvegarde").open(self.get_root(), None, self._on_pg_restore_selected)

    def _on_pg_restore_selected(self, dialog, result):
        try:
            file = dialog.open_finish(result)
        except Exception as e:
            self.terminal._log(f"❌ Erreur sélection fichier: {e}")
            return
        if not file:
            return
        archive_path = Path(file.get_path())

        cfg = self.get_config()
        db_name = cfg.get("pg_db_name", "ma_base")

        # Restaurer écrase les données existantes : confirmation obligatoire,
        # même schéma que pour "Tuer un processus" plus haut dans ce fichier.
        dlg = Adw.MessageDialog(
            transient_for=self.get_root(),
            heading=f"Restaurer « {db_name} » ?",
            body=f"Le contenu actuel de la base « {db_name} » sera remplacé par "
                 f"celui de « {archive_path.name} ». Cette action est irréversible.",
        )
        dlg.add_response("cancel", "Annuler")
        dlg.add_response("restore", "Restaurer")
        ...
        def on_resp(d, response):
            d.close()
            if response == "restore":
                self._do_pg_restore(archive_path, db_name)

        dlg.connect("response", on_resp)
        dlg.present()

    def _do_pg_restore(self, archive_path: Path, db_name: str):
        cfg = self.get_config()
        db_user = cfg.get("pg_db_user", "mon_user")
        self.terminal._log(f"♻️ === Restauration de « {db_name} » depuis {archive_path.name} ===")

        def _thread():
            try:
                with tempfile.TemporaryDirectory() as tmp_dir:
                    GLib.idle_add(self.terminal._log, "▶ Extraction de l'archive...")
                    with zipfile.ZipFile(archive_path, "r") as zf:
                        sql_names = [n for n in zf.namelist() if n.endswith(".sql")]
                        if not sql_names:
                            raise RuntimeError("Aucun fichier .sql trouvé dans cette archive.")
                        zf.extractall(path=tmp_dir)
                        sql_file = Path(tmp_dir) / sql_names[0]

                    # On repart d'une base vide avant de rejouer le dump, pour
                    # éviter les conflits avec les données déjà présentes.
                    GLib.idle_add(self.terminal._log, f"▶ Réinitialisation de « {db_name} »...")
                    subprocess.run(["sudo", "-u", "postgres", "dropdb", "--if-exists", db_name], check=True)
                    subprocess.run(["sudo", "-u", "postgres", "createdb", "-O", db_user, db_name], check=True)

                    GLib.idle_add(self.terminal._log, "▶ Import du dump SQL...")
                    result = subprocess.run(
                        ["sudo", "-u", "postgres", "psql", "-d", db_name, "-f", str(sql_file)],
                        capture_output=True, text=True,
                    )
                    if result.returncode != 0:
                        raise RuntimeError(result.stderr.strip() or "psql a échoué pendant l'import")

                GLib.idle_add(self.terminal._log, "✅ Restauration terminée.")
                GLib.idle_add(self.show_toast, f"♻️ Base « {db_name} » restaurée")
            except Exception as e:
                GLib.idle_add(self.terminal._log, f"❌ Erreur restauration: {e}")
                GLib.idle_add(self.show_toast, "❌ Échec de la restauration")

        threading.Thread(target=_thread, daemon=True).start()

    def _run_pg_reset(self, *_):
        cfg = self.get_config()
        db_name = cfg.get("pg_db_name", "ma_base")

        dlg = Adw.MessageDialog(
            transient_for=self.get_root(),
            heading=f"Réinitialiser « {db_name} » ?",
            body=f"TOUTES les données de la base « {db_name} » seront définitivement "
                 f"supprimées et remplacées par une base vide. Pense à faire une "
                 f"sauvegarde avant si besoin.",
        )
        dlg.add_response("cancel", "Annuler")
        dlg.add_response("reset", "Réinitialiser")
        ...
        def on_resp(d, response):
            d.close()
            if response == "reset":
                self._do_pg_reset(db_name)

        dlg.connect("response", on_resp)
        dlg.present()

    def _do_pg_reset(self, db_name: str):
        cfg = self.get_config()
        db_user = cfg.get("pg_db_user", "mon_user")
        self.terminal._log(f"🗑 === Réinitialisation de « {db_name} » ===")

        def _thread():
            try:
                GLib.idle_add(self.terminal._log, f"▶ Suppression de « {db_name} »...")
                subprocess.run(["sudo", "-u", "postgres", "dropdb", "--if-exists", db_name], check=True)
                GLib.idle_add(self.terminal._log, f"▶ Recréation vide de « {db_name} »...")
                subprocess.run(["sudo", "-u", "postgres", "createdb", "-O", db_user, db_name], check=True)
                GLib.idle_add(self.terminal._log, "✅ Base réinitialisée (vide).")
                GLib.idle_add(self.show_toast, f"🗑 Base « {db_name} » réinitialisée")
            except subprocess.CalledProcessError as e:
                GLib.idle_add(self.terminal._log, f"❌ Erreur: {e}")
                GLib.idle_add(self.show_toast, "❌ Échec de la réinitialisation")
            except Exception as e:
                GLib.idle_add(self.terminal._log, f"❌ Exception: {e}")
    ...
    def _run_nfs_server_start(self, *_):
        ...
        if mode == "network":
            lan_network = cfg.get("nfs_lan_network", "").strip()
            if not lan_network:
                self.show_toast("⚠️ Renseignez le réseau autorisé (Settings → NFS Serveur) avant de démarrer en mode Réseau")
                self.terminal._log("❌ Démarrage NFS annulé : aucun réseau autorisé configuré")
                return
        else:
            lan_network = "127.0.0.1"
    ...
    def _run_nfs_client_mount(self, *_):
        cfg = self.get_config()
        server_ip = cfg.get("nfs_client_server_ip", "").strip()
        if not server_ip:
            self.show_toast("⚠️ Renseignez l'IP du serveur NFS dans Settings → NFS Client")
            self.terminal._log("❌ Montage NFS annulé : aucune IP de serveur configurée")
            return
    ...
    def _show_nginx_config_dialog(self, *_):
        ...
        enable_window_controls(dialog, "⚙ Configuration Avancée Nginx")
        dialog.add_css_class("rounded-dialog")
...
client_max_body_size {max_body};
    ...
    def _show_ssh_config_dialog(self, *_):
        ...
        enable_window_controls(dialog, "⚙ Configuration SSH")
        dialog.add_css_class("rounded-dialog")
    ...
    def _show_venv_install_dialog(self, *_):
        ...
        enable_window_controls(dialog, "Installer Module Pip")
        dialog.add_css_class("rounded-dialog")
        dialog.set_default_size(400, 200)
        content = dialog.get_content_area()
        content.set_spacing(10)
    ...
    # ── OUTILS EXTERNES : AntiMicroX & pip install depuis un dossier ──
    def _show_antimicrox_dialog(self, *_):
        """Choisit un fichier de profil AntiMicroX (.amgp/.gamecontroller.amgp) puis
        lance AntiMicroX en arrière-plan (--hidden) avec ce profil chargé."""
        file_dialog = Gtk.FileDialog(title="Choisir un profil AntiMicroX")
        profile_filter = Gtk.FileFilter(); profile_filter.set_name("Profils AntiMicroX (*.amgp)"); profile_filter.add_pattern("*.amgp")
        all_filter = Gtk.FileFilter(); all_filter.set_name("Tous les fichiers"); all_filter.add_pattern("*")
        filter_list = Gio.ListStore.new(Gtk.FileFilter); filter_list.append(profile_filter); filter_list.append(all_filter)
        file_dialog.set_filters(filter_list)
        file_dialog.open(self.get_root(), None, self._on_antimicrox_profile_chosen)

    def _on_antimicrox_profile_chosen(self, dialog, result):
        try:
            file = dialog.open_finish(result)
            if not file:
                return
            profile_path = file.get_path()
        except Exception as e:
            global_log(f"⚠️ Erreur sélection profil AntiMicroX: {type(e).__name__} - {e}")
            return
        self._run_antimicrox_hidden(profile_path)

    def _run_antimicrox_hidden(self, profile_path: str):
        if not shutil.which("antimicrox"):
            self.show_toast("❌ AntiMicroX n'est pas installé ou introuvable dans le PATH")
            self.terminal._log("❌ Commande 'antimicrox' introuvable. Installez AntiMicroX d'abord.")
            return
        cmd = ["antimicrox", "--hidden", "--profile", profile_path]
        self.terminal._log(f"🎮 Lancement : {' '.join(cmd)}")
        try:
            subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
            self.show_toast(f"🎮 AntiMicroX lancé en arrière-plan avec « {Path(profile_path).name} »")
        except Exception as e:
            self.terminal._log(f"❌ Erreur lancement AntiMicroX: {e}")
            self.show_toast("❌ Échec du lancement d'AntiMicroX")

    def _show_pip_install_folder_dialog(self, *_):
        """Choisit un dossier contenant des paquets (.whl/.tar.gz/.zip) et les installe tous
        via pip — pratique pour une installation hors-ligne depuis une clé USB Gykhamine."""
        file_dialog = Gtk.FileDialog(title="Choisir un dossier de paquets pip")
        file_dialog.select_folder(self.get_root(), None, self._on_pip_folder_chosen)

    def _on_pip_folder_chosen(self, dialog, result):
        try:
            folder = dialog.select_folder_finish(result)
            if not folder:
                return
            ...
            global_log(f"⚠️ Erreur sélection dossier pip: {type(e).__name__} - {e}")
            return
        self._run_pip_install_from_folder(folder_path)

    def _run_pip_install_from_folder(self, folder_path: Path):
        packages = sorted(
            list(folder_path.glob("*.whl")) + list(folder_path.glob("*.tar.gz")) + list(folder_path.glob("*.zip"))
        )
        if not packages:
            self.show_toast(f"❌ Aucun paquet (.whl/.tar.gz/.zip) trouvé dans {folder_path.name}")
            return

        # Utilise le pip du venv du projet s'il existe, sinon celui de l'interpréteur courant
        pip_cmd = [sys.executable, "-m", "pip"]
        root = self.get_project_root()
        if root:
            cfg = self.get_config()
            venv_path = root / cfg.get("venv_name", "venv")
        ...
        self.terminal._log(f"📦 Installation de {len(packages)} paquet(s) depuis {folder_path} …")
        self.show_toast(f"⏳ Installation de {len(packages)} paquet(s)…")

        def _thread():
            try:
                cmd = pip_cmd + ["install"] + [str(p) for p in packages]
                result = subprocess.run(cmd, capture_output=True, text=True)
                GLib.idle_add(self.terminal._log, result.stdout)
                if result.returncode != 0:
                    GLib.idle_add(self.terminal._log, f"❌ {result.stderr}")
                    GLib.idle_add(self.show_toast, "❌ Échec de l'installation d'un ou plusieurs paquets")
                else:
                    GLib.idle_add(self.show_toast, f"✅ {len(packages)} paquet(s) installé(s)")
            except Exception as e:
                GLib.idle_add(self.terminal._log, f"❌ Erreur pip install: {e}")
                GLib.idle_add(self.show_toast, "❌ Échec de l'installation")
    ...
    def _open_prompt_manager(self, *_):
        dlg = PromptManagerDialog(self.get_root(), show_toast=self.show_toast)
        dlg.present()

    def _open_cache_manager(self, *_):
        dlg = CacheManagerDialog(self.get_root(), show_toast=self.show_toast)
        dlg.present()

    def _open_db_manager(self, *_):
        dlg = DbManagerDialog(self.get_root(), get_config=self.get_config, show_toast=self.show_toast, on_recent_changed=self.on_recent_changed)
        dlg.present()
        dlg.present()
...
"""
    ...
    def _display_db_stats_popup(self, stats: list):
        ...
        enable_window_controls(dialog, "📊 Visualisation des Tables et Données (Illimité)")
        dialog.add_css_class("rounded-dialog")
    ...
    def _on_gy_path_changed(self, key: str, entry):
        self.gy_paths[key] = entry.get_text().strip()
        cfg = self.get_config()
        cfg["gy_capsule_path_1" if key == "sudo" else "gy_capsule_path_2"] = self.gy_paths[key]
        save_config(cfg)
    ...
    def _open_gy_in_editor(self, key: str):
        root = self.get_project_root()
        if not root: return self.show_toast("⚠️ No project open")
        gy_path = root / self.gy_paths[key]
        if not gy_path.exists(): return self.show_toast(f"❌ Not found: {gy_path}")
        if self.on_open_in_editor:
            self.on_open_in_editor(gy_path)
        else:
            self.show_toast("⚠️ Block Editor unavailable")

    def _on_capsule_autorun_toggled(self, switch, state):
        cfg = self.get_config()
        cfg["gy_capsule_autorun"] = state
        save_config(cfg)
        return False
    ...
    def _compress_project(self, *_):
        ...
        enable_window_controls(dialog, "Save ZIP archive")
