...
class ControlPanel(Gtk.Box):
    def __init__(self, get_project_root, get_config, show_toast, terminal_panel, open_in_editor_cb=None):
        ...
        self.open_in_editor_cb = open_in_editor_cb
    ...
    def _build(self):
        ...
        for label, cb in [("🔍 Check", self._check_ports), ("🔫 Kill port", self._kill_port_dialog), ("🔓 Firewall Allow", self._ufw_allow_dialog), ("🌐 URLs Accessibles", self._show_open_browser_dialog)]:
        ...
        # Pour chaque script (1/gy.py = sudo, 2/gy.py = utilisateur normal) :
        # un champ de chemin éditable (persisté en DB, gy1_path/gy2_path — plus
        # aucun chemin figé dans le code), un bouton Lancer, un bouton Éditer
        # (ouvre le fichier dans l'éditeur de blocs), et une case "Auto au
        # démarrage" (gy1_auto_start / gy2_auto_start), consultée juste après
        # le montage du volume, avant même que le reste de l'interface se
        # construise.
        self._gy_auto_checks = {}
        self._gy_path_entries = {}
        for label, sudo, path_key, config_key in [
            ("🔑 /1/gy.py", True, "gy1_path", "gy1_auto_start"),
            ("👤 /2/gy.py", False, "gy2_path", "gy2_auto_start"),
        ]:
            current_path = self.get_config().get(path_key, "")

            path_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            path_row.append(Gtk.Label(label=label, css_classes=["dim-label"]))
            entry_path = Gtk.Entry()
            entry_path.set_text(current_path)
            entry_path.set_hexpand(True)
            entry_path.set_placeholder_text("Chemin relatif au projet, ex: Gykhamine/1/gy.py")
            entry_path.connect("changed", lambda e, k=path_key: self._on_gy_path_changed(k, e.get_text()))
            self._gy_path_entries[path_key] = entry_path
            path_row.append(entry_path)
            self.append(path_row)

            cap_row = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            cap_row.set_margin_bottom(4)
            btn_run = Gtk.Button(label=f"▶ Lancer")
            btn_run.add_css_class("ctrl-btn-warn" if sudo else "ctrl-btn")
            btn_run.set_hexpand(True)
            btn_run.connect("clicked", lambda *_, k=path_key, s=sudo: self._run_gy(self.get_config().get(k, ""), s))
            cap_row.append(btn_run)

            btn_edit = Gtk.Button(label="✏️ Éditer")
            btn_edit.add_css_class("ctrl-btn-small")
            btn_edit.connect("clicked", lambda *_, k=path_key: self._edit_gy(self.get_config().get(k, "")))
            cap_row.append(btn_edit)

            check_auto = Gtk.CheckButton(label="Auto")
            check_auto.set_tooltip_text("Lancer ce script automatiquement à chaque démarrage de Gykhamine Studio")
            check_auto.set_active(bool(self.get_config().get(config_key, False)))
            check_auto.connect("toggled", lambda cb, k=config_key: self._on_gy_auto_toggled(k, cb.get_active()))
            self._gy_auto_checks[config_key] = check_auto
            cap_row.append(check_auto)
    ...
    def _ufw_allow_dialog(self, *_):
        dialog = Gtk.Dialog(title="Ouvrir un port (firewalld)", transient_for=self.get_root()); dialog.add_css_class("rounded-dialog"); dialog.set_default_size(320, 160)
        ...
        def on_open(*_):
            try:
                ...
                self.terminal._log(f"🔓 Demande d'ouverture du port {port}/tcp via firewalld...")
                proc = subprocess.run(["firewall-cmd", "--permanent", f"--add-port={port}/tcp"], capture_output=True, text=True)
                if proc.returncode == 0:
                    subprocess.run(["firewall-cmd", "--reload"], capture_output=True, text=True)
                    self.terminal._log(f"✅ Port {port}/tcp ouvert avec succès."); self.show_toast(f"✅ Port {port} ouvert")
                else: self.terminal._log(f"❌ Erreur firewalld: {proc.stderr.strip() or proc.stdout.strip() or 'Erreur inconnue'}"); self.show_toast("❌ Échec de l'ouverture du port")
    ...
    def _build_firewall_page(self):
        ...
        btn_enable = Gtk.Button(label="✅ Activer Firewall", css_classes=["suggested-action"])
        btn_enable.connect("clicked", lambda *_: self._run_ufw_command("enable"))
        btn_disable = Gtk.Button(label="❌ Désactiver Firewall", css_classes=["destructive-action"])
    ...
    def _refresh_ufw_status(self):
        def _thread():
            try:
                ...
                rules = subprocess.run(["firewall-cmd", "--list-all"], capture_output=True, text=True)
                output = rules.stdout.strip() or rules.stderr.strip()
                GLib.idle_add(lambda: self.txt_fw_rules.get_buffer().set_text(output))
                if "running" in state.stdout.strip():
    ...
    def _run_ufw_command(self, cmd):
        # cmd: "enable" / "disable" démarrent ou arrêtent le service firewalld
        service_cmd = {"enable": "start", "disable": "stop"}.get(cmd, cmd)
        self.terminal._log(f"▶ sudo systemctl {service_cmd} firewalld")
        def _thread():
            try:
                import subprocess, gi
                gi.require_version("GLib", "2.0")
                from gi.repository import GLib
                proc = subprocess.run(["sudo", "systemctl", service_cmd, "firewalld"], capture_output=True, text=True)
                GLib.idle_add(lambda: self.terminal._log(proc.stdout + proc.stderr))
                GLib.idle_add(self._refresh_ufw_status)
                GLib.idle_add(lambda: self.show_toast(f"✅ Firewall {cmd} effectué"))
            except Exception as e:
                import gi
                gi.require_version("GLib", "2.0")
                from gi.repository import GLib
    ...
    def _fw_rule(self, port, proto, add: bool):
        if not port: return self.show_toast("❌ Port requis")
        action = "--add-port" if add else "--remove-port"
        label = "autorisation" if add else "interdiction"
        self.terminal._log(f"▶ sudo firewall-cmd --permanent {action}={port}/{proto}")
        def _thread():
            import subprocess, gi
            gi.require_version("GLib", "2.0")
            from gi.repository import GLib
            proc = subprocess.run(["sudo", "firewall-cmd", "--permanent", f"{action}={port}/{proto}"], capture_output=True, text=True)
            subprocess.run(["sudo", "firewall-cmd", "--reload"], capture_output=True, text=True)
            GLib.idle_add(lambda: self.terminal._log(proc.stdout + proc.stderr))
            GLib.idle_add(self._refresh_ufw_status)
            GLib.idle_add(lambda: self.show_toast(f"✅ Règle d'{label} appliquée ({port}/{proto})"))
        import threading
        threading.Thread(target=_thread, daemon=True).start()

    def _ufw_allow_port(self, port, proto):
        self._fw_rule(port, proto, add=True)

    def _ufw_deny_port(self, port, proto):
        self._fw_rule(port, proto, add=False)
    ...
    def _create_service_row(self, name, enabled_state):
        row = Gtk.ListBoxRow()
        box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        set_margins(box, 8)
        info_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=2)
        info_box.append(Gtk.Label(label=name, xalign=0, css_classes=["heading"]))
        # lbl_state est une variable LOCALE à cette ligne (pas self.lbl_state,
        # qui était réassigné à chaque ligne créée et finissait par ne
        # référencer que la toute dernière ligne de la liste — un bug qui
        # faisait que la mise à jour de statut d'une ligne pouvait modifier
        # le texte d'une autre ligne au hasard). Affiche maintenant le VRAI
        # état d'exécution (actif/inactif/échec), pas seulement l'activation
        # au démarrage — c'était l'information manquante signalée.
        lbl_state = Gtk.Label(label=f"⏳ Vérification…  •  Démarrage: {enabled_state}", xalign=0, css_classes=["dim-label"])
        info_box.append(lbl_state)
        box.append(info_box)
        spacer = Gtk.Box()
        ...
        btn_start.connect("clicked", lambda *_: self._control_service(name, "start", lbl_state, enabled_state))
        btn_restart = Gtk.Button(label="🔄"); btn_restart.set_tooltip_text("Redémarrer")
        btn_restart.connect("clicked", lambda *_: self._control_service(name, "restart", lbl_state, enabled_state))
        btn_stop = Gtk.Button(label="⏹"); btn_stop.set_tooltip_text("Arrêter")
        btn_stop.connect("clicked", lambda *_: self._control_service(name, "stop", lbl_state, enabled_state))
        ...
        btn_auto.connect("toggled", lambda b: self._toggle_service_auto_start(name, b.get_active(), lbl_state))
        ctrl_box.append(btn_start)
        ctrl_box.append(btn_restart)
        ctrl_box.append(btn_stop)
        ctrl_box.append(btn_auto)
        box.append(ctrl_box)
        row.set_child(box)
        # Interroge l'état d'exécution réel en arrière-plan, sans bloquer
        # l'affichage de la liste (potentiellement des dizaines de services).
        self._refresh_service_active_state(name, enabled_state, lbl_state)
        return row

    def _refresh_service_active_state(self, service_name, enabled_state, label_widget):
        """Interroge `systemctl is-active` pour ce service et met à jour le
        label avec le VRAI état d'exécution (actif ● / inactif ○ / échec ✗),
        en plus de l'état d'activation au démarrage déjà affiché."""
        def _thread():
            try:
                ...
                icon = {"active": "🟢", "inactive": "⚪", "failed": "🔴", "activating": "🟡"}.get(active_state, "❔")
                GLib.idle_add(lambda: label_widget.set_text(f"{icon} {active_state}  •  Démarrage: {enabled_state}"))
            except Exception as e:
                GLib.idle_add(lambda: label_widget.set_text(f"❔ Statut inconnu ({e})  •  Démarrage: {enabled_state}"))
        threading.Thread(target=_thread, daemon=True).start()
    ...
    def _toggle_service_auto_start(self, service_name, enable, label_widget):
        ...
        try:
            ...
            new_enabled_label = "activé" if enable else "désactivé"
            # Rafraîchit avec le VRAI statut actif/inactif en plus de l'état
            # au démarrage, pour ne pas perdre l'information ajoutée
            # précédemment (le simple "État au démarrage: ..." écrasait tout).
            GLib.idle_add(lambda: self._refresh_service_active_state(service_name, new_enabled_label, label_widget))
            GLib.idle_add(lambda: self.show_toast(f"✅ {service_name} {new_enabled_label} au démarrage"))
        except Exception as e:
            GLib.idle_add(lambda: self.terminal._log(f"❌ Erreur DB/Systemd: {e}"))

    def _control_service(self, service_name, action, label_widget=None, enabled_state=""):
        ...
        def _thread():
            try:
                ...
                # Rafraîchit le VRAI statut affiché après l'action, au lieu de
                # laisser le texte précédent figé (potentiellement obsolète
                # après un start/stop/restart).
                if label_widget is not None:
                    GLib.idle_add(lambda: self._refresh_service_active_state(service_name, enabled_state, label_widget))
            except Exception as e:
    ...
    def _build_dns_page(self):
        ...
        apply_dark_source_scheme(self.txt_current_dns.get_buffer())
...
"""
    ...
    def _build_dnf_page(self):
        ...
        apply_dark_source_scheme(self.txt_repo_log.get_buffer())
...
"""
    ...
    def _edit_gy(self, rel_path: str):
        """Ouvre directement le script (1/gy.py ou 2/gy.py) dans l'éditeur de
        blocs, exactement comme n'importe quel autre fichier du projet —
        remplace l'ancienne approche où ces scripts n'étaient éditables qu'en
        externe."""
        root = self.get_project_root()
        if not root:
            return self.terminal._log("❌ Aucun projet ouvert")
        gy_path = root / rel_path
        if not gy_path.exists():
            return self.terminal._log(f"❌ Introuvable: {gy_path}")
        if self.open_in_editor_cb:
            self.open_in_editor_cb(gy_path)
        else:
            self.terminal._log("⚠️ Édition indisponible (éditeur non connecté)")

    def _on_gy_auto_toggled(self, config_key: str, active: bool):
        """Enregistre en DB si ce script doit être relancé automatiquement au
        prochain démarrage de Gykhamine Studio (gy1_auto_start / gy2_auto_start).
        Lu juste après le montage du volume au démarrage de l'app — aucun
        scan, aucune automatisation complexe, juste un booléen consulté
        directement."""
        cfg = self.get_config()
        cfg[config_key] = active
        save_config(cfg)
        state = "activé" if active else "désactivé"
        self.terminal._log(f"⚙️ Lancement automatique {state} pour {config_key.replace('_auto_start', '')}")

    def _on_gy_path_changed(self, path_key: str, new_path: str):
        """Enregistre en DB le chemin (relatif au projet) du script capsule
        gy.py concerné (gy1_path / gy2_path), pour qu'il ne soit plus jamais
        codé en dur : modifiable directement depuis ce champ ou via le
        DB Manager (table config)."""
        cfg = self.get_config()
        cfg[path_key] = new_path
        save_config(cfg)
