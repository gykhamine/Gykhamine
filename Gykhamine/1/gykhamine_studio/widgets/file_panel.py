...
class FilePanel(Gtk.Box):
    def __init__(self, on_file_select, on_project_select, on_file_created, on_file_imported):
        ...
        # Raccourcis clavier locaux à l'explorateur (actifs uniquement quand le
        # tree_view a le focus, pour ne pas entrer en conflit avec le copier-coller
        # texte des éditeurs de blocs) : Suppr, F2, Ctrl+C/X/V pour agir sur un ou
        # plusieurs fichiers/dossiers sélectionnés.
        key_ctrl = Gtk.EventControllerKey()
        key_ctrl.connect("key-pressed", self._on_tree_key_pressed)
    ...
    def _reveal_path(self, target_path):
        """Développe l'arbre jusqu'au dossier indiqué et le sélectionne, afin que
        toute opération modifiant la structure (création, import, collage,
        renommage, suppression) ouvre automatiquement le dernier dossier concerné
        au lieu de laisser l'utilisateur le rechercher manuellement."""
        if not target_path:
            return
        target_str = str(target_path)
        tree_iter = self._tree_path_map.get(target_str)
        ...
        try:
            ...
            global_log(f"⚠️ Erreur reveal_path: {type(e).__name__} - {e}")

    def _refresh_and_reveal(self, target_path, select_file: bool = False):
        """Rafraîchit l'arbre puis ouvre automatiquement le dernier élément
        concerné par l'opération : le dossier lui-même s'il s'agit d'un dossier,
        sinon son dossier parent (select_file=False) ou le fichier exact
        (select_file=True, utile après une sauvegarde pour le mettre en évidence)."""
        ...
        if p.is_dir():
            ...
            reveal = p.parent
        GLib.idle_add(self._reveal_path, str(reveal))

    def _on_search_changed(self, entry):
        """Filtre l'arbre de fichiers selon la recherche et amène directement
        à la première correspondance trouvée, avec sélection et focus, au
        lieu de se contenter de déplier les dossiers correspondants."""
        ...
        if search_text:
            ...
            if self._first_match_path is not None:
                self.tree_view.expand_to_path(self._first_match_path)
                self.tree_view.set_cursor(self._first_match_path)
                self.tree_view.scroll_to_cell(self._first_match_path, None, True, 0.5, 0.0)
    ...
    def _expand_matching(self, model, path, tree_iter, search_text):
        """Expand paths that match the search, et retient le chemin de la
        toute première correspondance rencontrée pour lui donner le focus."""
        name = model.get_value(tree_iter, 0).lower()
        if search_text in name:
            if getattr(self, "_first_match_path", None) is None:
                self._first_match_path = path.copy()
            # Expand parent rows
    ...
    def _create_new_file(self, *_):
        ...
        apply_dark_source_scheme(text_buf)
        ...
        def on_create(*_):
            ...
            try:
                ...
                self.on_file_created(filepath); self._refresh_and_reveal(filepath); dialog.destroy()
            except Exception as e:
    ...
    def _create_new_folder(self, *_):
        if not self.project_root: return self._show_error("Aucun projet ouvert")
        dialog = Gtk.Dialog(title="Nouveau dossier", transient_for=self.get_root()); dialog.add_css_class("rounded-dialog"); dialog.set_default_size(350, 150)
        content = dialog.get_content_area(); content.set_spacing(8); set_margins(content, 12)
        content.append(Gtk.Label(label="Nom du dossier:", xalign=0))
        entry = Gtk.Entry(); entry.set_placeholder_text("ex: utils"); entry.set_activates_default(True); content.append(entry)
        btn_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        btn_cancel = Gtk.Button(label="Annuler"); btn_create = Gtk.Button(label="✅ Créer"); btn_create.add_css_class("suggested-action")
        btn_box.append(btn_cancel); btn_box.append(btn_create); content.append(btn_box)
        def on_create(*_):
            dirname = entry.get_text().strip()
            if not dirname: return self._show_error("Nom requis")
            dirpath = self.project_root / dirname
            if dirpath.exists(): return self._show_error(f"{dirname} existe déjà")
            try:
                dirpath.mkdir(parents=True)
                self._refresh_and_reveal(dirpath); dialog.destroy()
            except Exception as e:
                self._log_message(f"❌ Erreur création dossier: {e}")
                self._show_error(f"Erreur: {e}")
        btn_create.connect("clicked", on_create); btn_cancel.connect("clicked", lambda *_: dialog.destroy()); dialog.present()
        ...
        try:
            ...
            if file:
                ...
                if dst.exists() and Gtk.MessageDialog(transient_for=self.get_root(), flags=0, message_type=Gtk.MessageType.WARNING, buttons=Gtk.ButtonsType.YES_NO, text="Fichier existant", secondary_text=f"{dst.name} existe. Écraser ?").run() != Gtk.ResponseType.YES:
                    return
                shutil.copy2(src, dst); self.on_file_imported(dst); self._refresh_and_reveal(dst)
        except Exception as e: 
    ...
    def _on_right_click(self, gesture, n_press, x, y):
        ...
        # Si le clic droit tombe dans une sélection multiple existante, on la
        # préserve pour proposer un menu contextuel groupé au lieu de la
        # réduire à la seule ligne cliquée.
        if selection.path_is_selected(path) and selection.count_selected_rows() > 1:
            items = self._get_selected_items()
            self._show_group_context_menu(int(x), int(y), items)
            return
        self.tree_view.set_cursor(path)
    ...
    def _on_tree_key_pressed(self, controller, keyval, keycode, state):
        """Gère Suppr, F2 et Ctrl+C/X/V sur l'explorateur, avec prise en charge
        de la sélection multiple (déplacer/copier/supprimer plusieurs fichiers ou
        dossiers en une seule action)."""
        ...
        if ctrl and keyval in (Gdk.KEY_c, Gdk.KEY_C):
            self._copy_selected_to_clipboard(); return True
        if ctrl and keyval in (Gdk.KEY_x, Gdk.KEY_X):
            self._cut_selected_to_clipboard(); return True
        if ctrl and keyval in (Gdk.KEY_v, Gdk.KEY_V):
            self._paste_clipboard_selected(); return True
        return False

    def _get_selected_item(self):
        """Retourne (full_path, name, is_folder) pour la première ligne
        actuellement sélectionnée dans l'arbre, ou None si rien n'est sélectionné.
        Utilisé par les actions qui n'agissent que sur un seul élément (F2)."""
        items = self._get_selected_items()
        return items[0] if items else None

    def _get_selected_items(self):
        """Retourne la liste [(full_path, name, is_folder), ...] de toutes les
        lignes actuellement sélectionnées (la sélection multiple est activée sur
        l'arbre). Utilisé par les raccourcis et actions groupées (Suppr, Déplacer,
        Copier plusieurs fichiers/dossiers à la fois)."""
        selection = self.tree_view.get_selection()
        ...
        for path in paths:
            ...
            items.append((full_path, name, is_folder))
        return items

    def _delete_selected(self, *_):
        """Supprime le ou les éléments actuellement sélectionnés (raccourci Suppr)."""
        items = self._get_selected_items()
        if not items:
            return self._show_toast("⚠️ Aucun élément sélectionné")
        if len(items) == 1:
            full_path, name, is_folder = items[0]
            self._delete_item(full_path, name, is_folder)
        else:
            self._delete_items(items)

    def _rename_selected(self, *_):
        """Renomme l'élément actuellement sélectionné (raccourci F2). Le
        renommage ne s'applique volontairement qu'à un seul élément à la fois :
        renommer plusieurs fichiers en masse vers un nom unique n'a pas de sens."""
        items = self._get_selected_items()
        if not items:
            return self._show_toast("⚠️ Aucun élément sélectionné")
        if len(items) > 1:
            return self._show_toast("⚠️ Sélectionnez un seul élément pour le renommer")
        full_path, name, is_folder = items[0]
        self._rename_item(full_path, name, is_folder)
    ...
    def _paste_clipboard(self, target_dir, popover):
        ...
        if not self.clipboard_action or not self.clipboard_path:
            return
        self._paste_clipboard_items(self.clipboard_paths or [self.clipboard_path], target_dir, self.clipboard_action)
    ...
    def _show_context_menu(self, x, y, full_path, name, is_folder):
        ...
        if not is_folder:
            ...
            btn_new_folder_here = Gtk.Button(label="📁 Nouveau dossier ici"); btn_new_folder_here.set_halign(Gtk.Align.FILL); btn_new_folder_here.add_css_class("flat"); btn_new_folder_here.connect("clicked", lambda *_: self._create_folder_in_dir(target_dir, popover))
            box.append(btn_new_folder_here)
    ...
    def _show_group_context_menu(self, x, y, items):
        """Menu contextuel pour une sélection multiple : Copier / Couper / Coller
        / Supprimer s'appliquent à tous les éléments sélectionnés à la fois. Le
        renommage est volontairement exclu (renommer plusieurs fichiers vers un
        nom unique n'a pas de sens)."""
        popover = Gtk.Popover(); popover.set_parent(self.tree_view); popover.set_has_arrow(False)
        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=4); set_margins(box, 6)
        box.append(Gtk.Label(label=f"{len(items)} éléments sélectionnés", xalign=0, css_classes=["dim-label"]))

        paths = [full_path for full_path, _, _ in items]

        def do_copy(*_):
            self.clipboard_action = "copy"; self.clipboard_paths = paths; self.clipboard_path = paths[0]
            popover.popdown(); self._show_toast(f"✅ {len(items)} élément(s) copié(s)")
        def do_cut(*_):
            self.clipboard_action = "cut"; self.clipboard_paths = paths; self.clipboard_path = paths[0]
            popover.popdown(); self._show_toast(f"✅ {len(items)} élément(s) coupé(s)")
        def do_delete(*_):
            popover.popdown(); self._delete_items(items)

        btn_copy = Gtk.Button(label="📋 Copier tout"); btn_copy.set_halign(Gtk.Align.FILL); btn_copy.add_css_class("flat"); btn_copy.connect("clicked", do_copy)
        btn_cut = Gtk.Button(label="✂️ Couper tout"); btn_cut.set_halign(Gtk.Align.FILL); btn_cut.add_css_class("flat"); btn_cut.connect("clicked", do_cut)
        box.append(btn_copy); box.append(btn_cut)

        if self.clipboard_action and self.clipboard_paths:
            def do_paste(*_):
                popover.popdown()
                self._paste_clipboard_items(self.clipboard_paths, str(self.project_root), self.clipboard_action)
            btn_paste = Gtk.Button(label=f"📥 Coller ({len(self.clipboard_paths)} élément(s)) ici"); btn_paste.set_halign(Gtk.Align.FILL); btn_paste.add_css_class("flat"); btn_paste.add_css_class("suggested-action"); btn_paste.connect("clicked", do_paste)
            box.append(btn_paste)

        btn_delete = Gtk.Button(label=f"🗑 Supprimer tout ({len(items)})"); btn_delete.set_halign(Gtk.Align.FILL); btn_delete.add_css_class("flat"); btn_delete.add_css_class("destructive-action"); btn_delete.connect("clicked", do_delete)
        box.append(btn_delete); popover.set_child(box)
        rect = Gdk.Rectangle(); rect.x = x; rect.y = y; rect.width = 1; rect.height = 1; popover.set_pointing_to(rect); popover.popup()
    ...
    def _create_folder_in_dir(self, target_dir, popover):
        popover.popdown()
        dialog = Gtk.Dialog(title="Nouveau dossier", transient_for=self.get_root()); dialog.add_css_class("rounded-dialog"); dialog.set_default_size(350, 150)
        content = dialog.get_content_area(); content.set_spacing(8); set_margins(content, 12)
        content.append(Gtk.Label(label="Nom du dossier:", xalign=0))
        entry = Gtk.Entry(); entry.set_placeholder_text("dossier"); entry.set_activates_default(True); content.append(entry)
        btn_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        btn_cancel = Gtk.Button(label="Annuler"); btn_create = Gtk.Button(label="✅ Créer"); btn_create.add_css_class("suggested-action")
        btn_box.append(btn_cancel); btn_box.append(btn_create); content.append(btn_box)
        def on_create(*_):
            dirname = entry.get_text().strip()
            if not dirname: dialog.destroy(); return
            dirpath = Path(target_dir) / dirname
            try:
    ...
    def _rename_item(self, full_path, old_name, is_folder, popover=None):
        ...
        def on_rename(*_):
            ...
            try:
                Path(full_path).rename(new_path); self._refresh_and_reveal(new_path); self._show_toast(f"✅ Renommé en '{new_name}'")
            except Exception as e: 
    ...
    def _delete_item(self, full_path, name, is_folder, popover=None):
        if popover: popover.popdown()
        ...
        def on_delete(*_):
            try:
                ...
                self._refresh_and_reveal(parent_dir); self._show_toast(f"🗑 Supprimé: {name}")
            except Exception as e: 
    ...
    def _delete_items(self, items):
        """Supprime plusieurs fichiers/dossiers sélectionnés en une seule
        confirmation (raccourci Suppr avec sélection multiple, ou menu contextuel
        groupé)."""
        dialog = Gtk.Dialog(title="Confirmer la suppression", transient_for=self.get_root()); dialog.add_css_class("rounded-dialog"); dialog.set_default_size(380, 200)
        content = dialog.get_content_area(); content.set_spacing(8); set_margins(content, 12)
        content.append(Gtk.Label(label=f"Supprimer {len(items)} éléments ?", xalign=0, margin_bottom=4))
        names_preview = ", ".join(name for _, name, _ in items[:5])
        if len(items) > 5:
            names_preview += f", … (+{len(items) - 5})"
        content.append(Gtk.Label(label=names_preview, xalign=0, css_classes=["dim-label"], wrap=True))
        content.append(Gtk.Label(label="Cette action est irréversible.", xalign=0, css_classes=["dim-label"], margin_top=6))
        btn_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
        btn_cancel = Gtk.Button(label="Annuler"); btn_delete = Gtk.Button(label=f"🗑 Supprimer ({len(items)})"); btn_delete.add_css_class("destructive-action")
        btn_box.append(btn_cancel); btn_box.append(btn_delete); content.append(btn_box)
        def on_delete(*_):
            errors = []
            ...
            self._refresh_and_reveal(parent_dir)
            if errors:
                self._log_message("❌ Erreurs suppression: " + "; ".join(errors))
                self._show_error(f"{len(errors)} erreur(s) sur {len(items)}")
            else:
                self._show_toast(f"🗑 {len(items)} éléments supprimés")
            dialog.destroy()
        btn_delete.connect("clicked", on_delete); btn_cancel.connect("clicked", lambda *_: dialog.destroy()); dialog.present()

    def _copy_selected_to_clipboard(self, *_):
        """Copie le ou les éléments sélectionnés dans le presse-papiers interne
        (Ctrl+C), pour un futur collage groupé (Ctrl+V)."""
        items = self._get_selected_items()
        if not items:
            return self._show_toast("⚠️ Aucun élément sélectionné")
        self.clipboard_action = "copy"
        self.clipboard_paths = [full_path for full_path, _, _ in items]
        self.clipboard_path = self.clipboard_paths[0] if self.clipboard_paths else None
        self._show_toast(f"✅ {len(items)} élément(s) copié(s)")

    def _cut_selected_to_clipboard(self, *_):
        """Coupe le ou les éléments sélectionnés (Ctrl+X), pour un déplacement
        groupé au prochain collage (Ctrl+V)."""
        items = self._get_selected_items()
        if not items:
            return self._show_toast("⚠️ Aucun élément sélectionné")
        ...
        self.clipboard_paths = [full_path for full_path, _, _ in items]
        self.clipboard_path = self.clipboard_paths[0] if self.clipboard_paths else None
        self._show_toast(f"✅ {len(items)} élément(s) coupé(s)")

    def _paste_clipboard_selected(self, *_):
        """Colle le contenu du presse-papiers interne dans le dossier
        actuellement sélectionné (Ctrl+V), en supportant un ou plusieurs
        fichiers/dossiers copiés ou coupés d'un coup."""
        if not self.clipboard_action or not getattr(self, "clipboard_paths", None):
            return self._show_toast("⚠️ Presse-papiers vide")
        item = self._get_selected_item()
        target_dir = item[0] if item and item[2] else str(self.project_root)
        self._paste_clipboard_items(self.clipboard_paths, target_dir, self.clipboard_action)

    def _paste_clipboard_items(self, src_paths, target_dir, action):
        """Copie ou déplace une liste de chemins vers target_dir en une seule
        opération, avec un unique rapport d'erreurs regroupé."""
        errors = []
        last_dst = None
        for src_str in src_paths:
            ...
            if src.resolve() == dst.resolve() or str(src.resolve()) in str(dst.resolve()):
                errors.append(f"{src.name}: impossible de coller dans lui-même")
                continue
            try:
                if action == "copy":
        ...
        if action == "cut" and not errors:
            self.clipboard_action = None
            self.clipboard_paths = []
            self.clipboard_path = None

        if last_dst:
            self._refresh_and_reveal(last_dst)
        if errors:
            self._log_message("❌ Erreurs collage: " + "; ".join(errors))
            self._show_error(f"{len(errors)} erreur(s) sur {len(src_paths)}")
        else:
            verb = "Copié" if action == "copy" else "Déplacé"
            self._show_toast(f"✅ {verb} : {len(src_paths)} élément(s)")