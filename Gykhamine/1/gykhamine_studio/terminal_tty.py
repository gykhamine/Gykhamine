...
class NativeTtyTerminal(Gtk.Window):
    def __init__(self, parent, title, command, cwd=None):
        super().__init__(title=title, transient_for=parent, default_width=960, default_height=650)
        self.add_css_class("rounded-dialog")
        # La fenêtre embarque déjà sa propre HeaderBar (avec le bouton
        # "Fermer") juste en dessous : sans set_decorated(False), GTK ajoute
        # EN PLUS sa barre de titre système qui reprend le même texte "title"
        # -> double titre superposé.
