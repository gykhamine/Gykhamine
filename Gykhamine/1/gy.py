...
class ControlPanel(Gtk.Box):
    ...
    def _show_system_manager_dialog(self, *_):
        dialog = Gtk.Dialog(title="🛡️ Gestion Système & Réseau", transient_for=self.get_root())
        dialog.add_css_class("rounded-dialog")
        ...
        notebook.append_page(self._build_firewall_page(), Gtk.Label(label="🔥 Firewall (UFW)"))
        notebook.append_page(self._build_network_page(), Gtk.Label(label="🌐 Réseau (IP/DHCP)"))
        notebook.append_page(self._build_systemd_page(), Gtk.Label(label="⚙️ Services Systemd"))
...
class CCompilerDialog(Gtk.Dialog):
    ...
    def _show_system_manager_dialog(self, *_):
        """Ouvre le panneau de gestion système complet"""
        dialog = Gtk.Dialog(title="🛡️ Gestion Système & Réseau", transient_for=self.get_root())
        dialog.add_css_class("rounded-dialog")
        dialog.set_default_size(900, 700)
        ...
        page_fw = self._build_firewall_page()
        notebook.append_page(page_fw, Gtk.Label(label="🔥 Firewall (UFW)"))
        
        page_net = self._build_network_page()
        notebook.append_page(page_net, Gtk.Label(label="🌐 Réseau (IP/DHCP)"))
        
        page_svc = self._build_systemd_page()
        notebook.append_page(page_svc, Gtk.Label(label="⚙️ Services Systemd"))
    ...
    def _apply_network_config(self):
        ...
        if "DHCP" in mode:
            ...
            # Construction robuste du script bash (concaténation explicite pour éviter les erreurs de syntaxe)
            dns_entries = "\n".join([f"nameserver {d.strip()}" for d in dns.split(',')])
            script = (
                f"ip addr flush dev {iface}\n"
                f"ip addr add {ip}/{mask} dev {iface}\n"
                f"ip route add default via {gw}\n"
                f"printf '%s\n' '{dns_entries}' > /etc/resolv.conf\n"
            )
            self._run_network_script(script)
    ...
    def _load_systemd_services(self):
        def _thread():
            try:
                ...
                proc = subprocess.run(["systemctl", "list-unit-files", "--type=service", "--no-pager", "--no-legend"], 
    ...
    def _filter_services(self):
        ...
        for name, state in self.all_services:
            if query in name.lower():
                row = self._create_service_row(name, state)
                self.listbox_services.append(row)
    ...
    def _create_service_row(self, name, state):
        ...
        lbl_state = Gtk.Label(label=f"État au démarrage: {state}", xalign=0, css_classes=["dim-label"])
        ...
        btn_start.set_tooltip_text("Démarrer")
        btn_start.connect("clicked", lambda *_: self._control_service(name, "start"))
        
        btn_stop = Gtk.Button(label="⏹")
        btn_stop.set_tooltip_text("Arrêter")
        btn_stop.connect("clicked", lambda *_: self._control_service(name, "stop"))
        
        btn_auto = Gtk.ToggleButton(label="Auto")
        btn_auto.set_tooltip_text("Activer/Désactiver au démarrage (Sauvegardé en DB)")
        
        is_enabled = self._is_service_auto_start(name)
        btn_auto.set_active(is_enabled)
    ...
    def _is_service_auto_start(self, service_name):
        ...
        try:
            proc = subprocess.run(["systemctl", "is-enabled", service_name], capture_output=True, text=True)
    ...
    def _toggle_service_auto_start(self, service_name, enable, label_widget):
        ...
        try:
            ...
            cur = con.cursor()
            cur.execute("CREATE TABLE IF NOT EXISTS liveos_services (name TEXT PRIMARY KEY, auto_start INTEGER DEFAULT 0)")
            cur.execute("INSERT OR REPLACE INTO liveos_services (name, auto_start) VALUES (?, ?)", 
                        (service_name, 1 if enable else 0))
            con.commit()
            con.close()
    ...
    def _apply_network_config(self):
        ...
        if "DHCP" in mode:
            ...
            script = (
                ...
                f"ip addr add {ip}/{mask} dev {iface}\n"
                f"ip route add default via {gw}\n"
                f"echo '{dns_text}' > /etc/resolv.conf\n"
            )
            self._run_network_script(script)
    ...
    def _toggle_service_auto_start(self, service_name, enable, label_widget):
        ...
        try:
            ...
            con.commit()
            con.close()
            subprocess.run(["sudo", "systemctl", "enable" if enable else "disable", service_name], capture_output=True)
            GLib.idle_add(lambda: label_widget.set_text(f"État au démarrage: {'activé' if enable else 'désactivé'} (DB)"))
            GLib.idle_add(lambda: self.show_toast(f"✅ {service_name} {'activé' if enable else 'désactivé'} au démarrage"))
        except Exception as e:
            import gi
    ...
    def _apply_network_config(self):
        ...
        if "DHCP" in mode:
            ...
            # Construction robuste du script bash (concaténation explicite pour éviter TOUTE erreur de syntaxe)
            dns_list = [f"nameserver {d.strip()}" for d in dns.split(',')]
            dns_text = "\n".join(dns_list)
